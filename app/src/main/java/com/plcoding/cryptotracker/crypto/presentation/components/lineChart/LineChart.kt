package com.plcoding.cryptotracker.crypto.presentation.components.lineChart
...
fun LineChart(
    ...
    val maxYValue = remember(visibleDataPoints) {
        visibleDataPoints.maxOfOrNull { it.y } ?: 0
    }

    val minYValue = remember(visibleDataPoints) {
        visibleDataPoints.minOfOrNull { it.y } ?: 0
    }
    ...
    Canvas(
        ...
        val valueIncrement = (maxYValue.toFloat() - minYValue.toFloat()) / labelCountExcludingLastLabel

        val yLabels = (0..labelCountExcludingLastLabel).map {
            ValueLabel(
                value = maxYValue.toFloat() - (valueIncrement * it).toFloat(),
                unit = unit
            )
        }
        ...
        drawPoints = visibleDataPointsIndices.map {
            ...
            val ratio =
                (dataPoints[it].y - minYValue.toFloat()) / (maxYValue.toFloat() - minYValue.toInt())
